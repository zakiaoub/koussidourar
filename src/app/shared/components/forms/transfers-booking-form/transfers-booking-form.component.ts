...
import { CheckoutTransferRecapComponent } from '@features/transfers/components/checkout-transfer-recap/checkout-transfer-recap.component'
...
@Component({
  selector: 'app-transfers-booking-form',
  imports: [CommonModule, TranslationModule, FormsModule, DropdownModule, TextareaModule, ReactiveFormsModule, ButtonComponent, SessionTimeComponent, CheckboxModule, CheckoutTransferRecapComponent, ProductCancellationPolicyComponent, BookingPaxesFieldsComponent, BookingContactFieldsComponent, SkeletonBookingComponent, SessionExpiredComponent, StepperComponent, ErrorRequestComponent],
  templateUrl: './transfers-booking-form.component.html',
  styleUrl: './transfers-booking-form.component.css'
})

export class TransfersBookingFormComponent {
  constructor(
  ...
  addToCart() {
    ...
    this.api.post(['transfer', 'prebooking', this.searchToken, this.rateKey].join('/'), this.dataBook()).subscribe({
      next: (response: any) => {
